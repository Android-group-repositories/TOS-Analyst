...
for DATA
若 文中出现了 "数据" 和对 "数据具体项的解释" 则两者都需要提取                
...
-<text>     :The use of Dropbox login credentials is only allowed in connection with the initial user authentication
-pattern 4  :your app {can} {use Dropbox login credentials} ;  {user} ;when {allowed in connection with user authentication}


-<text>     :The handling of Dropbox login credentials is restricted to within the developer's app only
-pattern 4  :your app {can} {handle Dropbox login credentials} ; when {within the developer's app only}

-<text>     :No sensitive personal information should be used with the service, except as permitted under section 4.5 (Protected Health Information).
-pattern 4  :your app {cannot} {use sensitive personal information with sdk-service} ; {user:all users} ; Even obtain user consent
...
The output only includes the part after "-Output" and pattern
be like:
"#Type2-pattern2# your app {can} {use service} ;{user:under the age of 16. } ;  Even obtain user consent "
