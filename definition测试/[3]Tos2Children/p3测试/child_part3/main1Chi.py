...
def prmopt3(history,data_dict):
    ...
    #     })
    filename = "child输出.txt"
    with open(filename, 'w') as file:
            pass  # 打开文件并清空内容，不需要写入任何内容
    # history_typy = init_type_prompt(history,P3_promptType1)
    history_1 = init_type_prompt(history,P3_promptType1)    
    history_2 = init_type_prompt(history,P3_promptType2)
...
Here are the <Text> you need to analyze:\n
                ...
                tmp = chat(query, history_1,False)
                print(tmp,file=f)
...
Here are the <Text> you need to analyze:\n
                ...
                tmp = chat(query, history_2,False)
                print(tmp,file=f)
...
def main():
    ...
    json_path = "biaoge_children.json"
